...
const AdminRewards = () => {
  ...
  const createPeriod = async () => {
    ...
    await supabase.from('reward_periods').insert({ period, status: 'open' });
    await loadPeriods();
  };
  ...
  const processRewards = async (periodId: string, periodStr: string) => {
    ...
    for (const div of divisions) {
      ...
      for (let i = 0; i < topPlayers.length && i < tiers.length; i++) {
        // Check if player has flagged matches
        const { data: flagged } = await supabase
          .from('match_logs')
          .select('id')
          .eq('player_id', topPlayers[i].player_id)
          .eq('is_flagged', true)
          .limit(1);

        if (flagged && flagged.length > 0) continue; // skip flagged players

        await supabase.from('reward_payouts').insert({
          reward_period_id: periodId,
          player_id: topPlayers[i].player_id,
          division: div,
          rank: i + 1,
          reward_amount_cents: tiers[i],
          status: 'pending',
        });
      }
    }
  ...
  const exportCSV = () => {
    const payoutData: PayoutData[] = payouts.map(p => ({
      ...
      division: p.division,
      rank: p.rank,
      amountCents: p.reward_amount_cents,
    }));
    ...
    a.href = url;
    a.download = `payouts_${selectedPeriod}.csv`;
    a.click();
    ...
    <div className="min-h-screen bg-[#0a0a1a] text-white font-mono p-6">
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="text-white/40 border-b border-white/10">
                  ...
                  <th className="py-1 px-2 text-left">Flags</th>
                </tr>
              </thead>
              <tbody>
                {flaggedMatches.slice(0, 10).map((m: any) => (
      ...
      <section className="mb-8">
        <div className="flex items-center gap-4 mb-3">
          <h2 className="text-sm uppercase tracking-widest text-yellow-400/70">Reward Periods</h2>
          <button
            onClick={createPeriod}
            className="text-[10px] px-2 py-1 rounded border border-yellow-500/30 text-yellow-400 hover:bg-yellow-500/10"
          >
            + New Period
          </button>
        </div>
          ...
          {periods.map(p => (
            <div key={p.id} className="flex items-center gap-3 text-xs">
              ...
              <span className={`px-1.5 py-0.5 rounded text-[10px] ${
                ...
                p.status === 'validating' ? 'bg-yellow-500/20 text-yellow-400' :
                'bg-blue-500/20 text-blue-400'
              }`}>{p.status}</span>
              {p.status === 'open' && (
                <button
                  onClick={() => processRewards(p.id, p.period)}
                  className="text-[10px] px-2 py-0.5 rounded border border-blue-500/30 text-blue-400 hover:bg-blue-500/10"
                >
                  Process Rewards
                </button>
              )}
              <button
                onClick={() => loadPayouts(p.id)}
                className="text-[10px] px-2 py-0.5 rounded border border-white/20 text-white/60 hover:bg-white/5"
              >
                View Payouts
              </button>
            </div>
          ...
          {payouts.length === 0 ? (
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="text-white/40 border-b border-white/10">
                  <th className="py-1 px-2 text-left">Player</th>
                  <th className="py-1 px-2 text-left">Division</th>
                  <th className="py-1 px-2 text-right">Rank</th>
